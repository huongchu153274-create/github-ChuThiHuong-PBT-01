// PHIẾU BÀI TẬP 08 — PHẦN B1: QUẢN LÝ SẢN PHẨM E-COMMERCE
...
console.log(`Total: ${getInStock(products).length} sản phẩm\n`);

console.log("=== PHONES 15-25 TRIỆU ===");
const phonesRange = filterProducts(products, "phone", 15000000, 25000000);
console.log(phonesRange);
console.log(`Total: ${phonesRange.length} sản phẩm\n`);

console.log("=== SORT BY PRICE (ASC) ===");
console.log(sortByPrice(products, "asc").map(p => `${p.name}: ${p.price.toLocaleString()}`));
console.log();

console.log("=== SORT BY PRICE (DESC) ===");
console.log(sortByPrice(products, "desc").map(p => `${p.name}: ${p.price.toLocaleString()}`));
console.log();

console.log("=== CHEAPEST BY CATEGORY ===");
const cheapest = cheapestByCategory(products);
Object.entries(cheapest).forEach(([cat, prod]) => {
    console.log(`${cat}: ${prod.name} - ${prod.price.toLocaleString()}đ`);
});
console.log();

console.log("=== TOTAL INVENTORY VALUE ===");
const totalValue = totalInventoryValue(products);
console.log(`Tổng giá trị kho: ${totalValue.toLocaleString()}đ\n`);

console.log("=== FORMATTED PRODUCT LIST ===");
console.log(formatProductList(products).slice(0, 3));
console.log("...\n");

console.log("=== AVERAGE RATING ===");
console.log(`Đánh giá trung bình: ${averageRating(products)} ⭐\n`);

console.log("=== SEARCH: 'Pro' ===");
